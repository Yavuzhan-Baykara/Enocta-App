    ...
    messages = [
        {
            "role": "system",
            "content": (
                "Sen bir liderlik koçu cevaplamak için bir asistansınız. "
                "Sorulara detaylı ve içgörülü Türkçe cevaplar ver. "
                "Bilgileri detaylıca anlat. "
                "Cevabı bilmiyorsanız, sadece 'Bilmiyorum' deyin. Bir cevap uydurmayın."
            )
        },
...
def process_instruction(instruction):
    ...
    # Bağlamı oluştur: BM25 skoru 0.3'ün altında ise yalnızca web araması sonucu,
    # 0.3'ün üzerindeyse BM25'dan gelen bağlamla web araması sonucunun snippet'lerini birleştir.
    if max_score < threshold:
        logging.info("BM25 skoru 0.3'ün altında, sadece web araması sonucu kullanılacak.")
        web_context = "\n".join([res.get("snippet", "") for res in web_results]) if web_results else ""
        context = web_context
    else:
        logging.info("BM25 skoru 0.3'ün üzerinde, BM25 bağlamı ve web araması sonucu birleştirilecek.")
        bm25_context = "\n".join(best_chunks)
        web_context = "\n".join([res.get("snippet", "") for res in web_results]) if web_results else ""
